...
namespace Movies
{
    ...
    public static class MovieDatabase
    {
        ...
        /// <summary>
        /// Filters the provided collection of movies by MPAA Ratings
        /// </summary>
        ...
        /// <summary>
        /// Filters the provided collection of movies by the Genre
        /// </summary>
        /// <param name="movies">The collection of movies to filter</param>
        /// <param name="ratings">The genres to include</param>
        /// <returns>A collection containing only movies that match the filter</returns>
        public static IEnumerable<Movie> FilterByGenre(IEnumerable<Movie> movies, IEnumerable<string> movieGenres) 
        {
            if (movieGenres == null || movieGenres.Count() == 0) { return movies; }

            // Filter the supplied collection of movies
            List<Movie> results = new List<Movie>();
            foreach (Movie movie in movies)
            {
                if (movie.MajorGenre != null && movieGenres.Contains(movie.MajorGenre))
                {
                    results.Add(movie);
                }
            }
        ...
        {
            // neither maximum or minimum specified
            if (min == null && max == null) return movies;
